...
int main(int argc, char **argv)
{
	try
	{
		...
		for (size_t i = 0; i < vFoundDevices.size(); ++i)
		{
			...
			std::cout << "  GPU-" << i << ": " << strName << ",显存" << globalMemSize << " bytes available, " << computeUnits << "  ���㵥Ԫ  (��ǰ����ʹ��Ԥ�����kernel��" << (precompiled ? "yes" : "no") << ")" << std::endl;
